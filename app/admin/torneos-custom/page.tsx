...
import { ArrowBack, Person, Search, SportsEsports } from '@mui/icons-material'
import {
...
function placementSummary(
  ...
  if (!mp) return '—'
  const cat =
    CATEGORY_LABELS[mp.categoryIndex] ?? `Cat. ${mp.categoryIndex + 1}`
...
export default function AdminTorneosCustomPage() {
  ...
  const rows = useMemo(() => data?.tournaments ?? [], [data?.tournaments])
  const filtered = useMemo(() => filterTournaments(rows, query), [rows, query])
                    ...
                    {filtered.map(row => {
                      ...
                      return (
                        <TableRow key={row._id} hover>
                          ...
                          <TableCell>
                            {cp ? placementSummary(cp.manualPlacement) : '—'}
                          </TableCell>
                          <TableCell>
                            <DeckSprites slugs={cp?.deckPokemonSlugs ?? []} />
                          </TableCell>
