  ...
  return {
    ...
    countBestEvents:
      doc.countBestEvents === null || doc.countBestEvents === undefined
        ? null
        : typeof doc.countBestEvents === "number" && Number.isFinite(doc.countBestEvents)
...
export async function GET() {
  try {
    ...
    const leagues = raw.map((d) =>
      serializeLeague(d as Record<string, unknown>),
    );
...
export async function POST(request: NextRequest) {
  try {
    ...
    const pointsByPlace =
      readPointsByPlace(body.pointsByPlace) ?? [...DEFAULT_LEAGUE_POINTS_BY_PLACE];
    ...
    const doc = await League.create({
      ...
      pointsByPlace,
      countBestEvents:
        countBestEvents === undefined ? null : countBestEvents,
    });

    return NextResponse.json(
      { league: serializeLeague(doc.toObject() as unknown as Record<string, unknown>) },
      { status: 201 },
    );
  } catch (error: unknown) {
    const code =
