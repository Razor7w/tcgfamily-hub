...
async function findUserByRut(input: string) {
  ...
  const user =
    (await User.findOne({ rut: { $in: [formattedDots, formattedNoDots, cleaned] } })) ??
    (await User.findOne({ rut: { $regex: `^${formattedDots}$`, $options: "i" } })) ??
    (await User.findOne({ rut: { $regex: `^${formattedNoDots}$`, $options: "i" } })) ??
