...
export function isRoundMatchWin(r: ParticipantMatchRoundDTO): boolean {
  ...
  if (
    r.specialOutcome === 'intentional_draw' ||
    r.specialOutcome === 'no_show'
  ) {
    return false
  }
...
export function isRoundMatchLoss(r: ParticipantMatchRoundDTO): boolean {
  if (r.specialOutcome === 'bye' || r.specialOutcome === 'intentional_draw')
    return false
...
export function parseParticipantMatchRoundsFromLean(
  ...
  for (const mr of raw) {
    ...
    const gameResults = Array.isArray(m.gameResults)
      ? (m.gameResults.filter(
          (g): g is 'W' | 'L' | 'T' => g === 'W' || g === 'L' || g === 'T'
        ) as ('W' | 'L' | 'T')[])
      : []
    const turnOrders = Array.isArray(m.turnOrders)
      ? (m.turnOrders.filter(
          (t): t is 'first' | 'second' => t === 'first' || t === 'second'
        ) as ('first' | 'second')[])
      : []
    const special =
...
export function matchRecordFromRounds(rounds: ParticipantMatchRoundDTO[]): {
  wins: number
  losses: number
  ties: number
} {
