    ...
    async linkAccount({ user, profile }) {
      ...
      if (profile && user.id) {
        ...
        if (existingUser) {
          ...
          if (profileData.name && typeof profileData.name === 'string' && profileData.name !== existingUser.name) {
            existingUser.name = profileData.name;
          }
          
          // Google usa 'image' para la URL de la imagen del perfil
          if (profileData.image && typeof profileData.image === 'string' && profileData.image !== existingUser.image) {
