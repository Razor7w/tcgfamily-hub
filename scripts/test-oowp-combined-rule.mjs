  ...
  for (const [pop, tom] of Object.entries(TOM)) {
    ...
    if (Math.abs(oo - tom) < 0.02) hit++
      ...
      miss.push({
        ...
        detail: os.map(
          o =>
            `${o}:${(tb.get(o).owp * 100).toFixed(1)}→${(adjFn(o, tb.get(o).owp) * 100).toFixed(1)}`
        )
      })
  }
  console.log(`\n${label}: ${hit}/17`)
  for (const m of miss)
    console.log(' ', m.pop, m.oo, 'TOM', m.tom, m.detail?.join(' '))
}
...
score('drop max75', (o, raw) =>
  dropped.has(o) ? Math.max(raw, 0.75) : raw < 0.5 ? 0.5 : raw
)
